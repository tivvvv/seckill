package com.tiv.seckill.goods.domain.service.impl;
...
public class SeckillGoodsDomainServiceImpl implements SeckillGoodsDomainService {
    ...
    public boolean increaseAvailableStock(Long id, Integer count) {
        ...
        boolean updateSuccess = seckillGoodsRepository.increaseAvailableStock(id, count) > 0;
        if (updateSuccess) {
            SeckillGoodsEvent seckillGoodsEvent = new SeckillGoodsEvent(id, seckillGoods.getStatus(), seckillGoods.getActivityId());
            eventPublisher.publish(seckillGoodsEvent);
            log.info("goodsPublish|增加秒杀商品库存事件发布成功|{}", id);
        } else {
            log.info("goodsPublish|秒杀商品库存未增加|{}", id);
        }
        return updateSuccess;
    }
